...
class db:
    ...
    def __init__(self, name_, pathway_):
        ...
        try:
            ...
            print(f'::ERROR:: a table in database config {self.dbtables} >>not<< defined int \"custom_tables\"')
            exit()
        ...
        for item in self.vars:
            try:
                ...
                print(f'::ERROR:: config variable in \"custom_tables\" >>not<< defined in \"custom_vars\": {item}')
                exit()
        self.types = t
...
def ret_vars(database, table):
    try:
        tb = tables_w_vars[table]
    except KeyError:
        print(f"::ERROR:: the table you passed does not exist: {table}")
    return tb
...
def input_one(db_name, table, insert_tuple):
    ...
    db_tables = ret_vars(db_name, table)
    par = generate_blank_par(len(db_tables))
    ...
    try:
        ...
        print(e)
        print(f"""INSERT INTO {table} VALUES{par}""", insert_tuple)
        return False
